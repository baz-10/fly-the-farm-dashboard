...
async function fetchOpenMeteoOperationsForecast({latitude,longitude,fetchImpl=global.fetch}){
  ...
  const d=await response.json(),compass=deg=>['N','NE','E','SE','S','SW','W','NW'][Math.round(Number(deg||0)/45)%8],allHourly=(d.hourly?.time||[]).map((time,i)=>{const deltaT=calculateDeltaT(d.hourly.temperature_2m[i],d.hourly.relative_humidity_2m[i]),trend=i?d.hourly.temperature_2m[i]-d.hourly.temperature_2m[i-1]:0,inversionPotential=assessInversionPotential({isDay:Boolean(d.hourly.is_day?.[i]),windSpeedKmh:d.hourly.wind_speed_10m[i],cloudCoverPercent:d.hourly.cloud_cover?.[i]??100,humidityPercent:d.hourly.relative_humidity_2m[i],temperatureTrendC:trend});return{time,temperatureC:d.hourly.temperature_2m[i],humidityPercent:d.hourly.relative_humidity_2m[i],windSpeedKmh:d.hourly.wind_speed_10m[i],windGustKmh:d.hourly.wind_gusts_10m[i],windDirection:compass(d.hourly.wind_direction_10m[i]),rainProbability:d.hourly.precipitation_probability[i],rainAmountMm:d.hourly.precipitation[i],deltaTC:deltaT,inversionPotential,sprayCondition:assessSprayCondition({deltaT,windSpeedKmh:d.hourly.wind_speed_10m[i],windGustKmh:d.hourly.wind_gusts_10m[i],rainProbability:d.hourly.precipitation_probability[i]})};}),startIndex=Math.max(0,allHourly.findIndex(item=>item.time>=d.current.time)),hourly=allHourly.slice(startIndex,startIndex+25),currentDelta=calculateDeltaT(d.current.temperature_2m,d.current.relative_humidity_2m),daily=(d.daily?.time||[]).map((date,i)=>({date,minTemperatureC:d.daily.temperature_2m_min[i],maxTemperatureC:d.daily.temperature_2m_max[i],rainProbability:d.daily.precipitation_probability_max[i],rainAmountMm:d.daily.precipitation_sum[i],windSpeedKmh:d.daily.wind_speed_10m_max[i],windGustKmh:d.daily.wind_gusts_10m_max[i]})),currentInversion=hourly[0]?.inversionPotential;
  return{provider:'Open-Meteo',retrievedAt:new Date().toISOString(),timezone:d.timezone,latitude:Number(d.latitude??latitude),longitude:Number(d.longitude??longitude),current:{time:d.current.time,temperatureC:d.current.temperature_2m,apparentTemperatureC:d.current.apparent_temperature,humidityPercent:d.current.relative_humidity_2m,windSpeedKmh:d.current.wind_speed_10m,windGustKmh:d.current.wind_gusts_10m,windDirection:compass(d.current.wind_direction_10m),rainAmountMm:Number(d.current.precipitation||d.current.rain||0),rainProbability:hourly[0]?.rainProbability??0,deltaTC:currentDelta,inversionPotential:currentInversion,condition:'Current conditions',minTemperatureC:daily[0]?.minTemperatureC,maxTemperatureC:daily[0]?.maxTemperatureC,sprayCondition:assessSprayCondition({deltaT:currentDelta,windSpeedKmh:d.current.wind_speed_10m,windGustKmh:d.current.wind_gusts_10m,rainProbability:hourly[0]?.rainProbability??0})},hourly,daily,bestSprayWindow:findBestSprayWindow(hourly)};
}
