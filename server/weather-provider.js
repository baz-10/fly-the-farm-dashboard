...
async function fetchOpenMeteoOperationsForecast({latitude,longitude,fetchImpl=global.fetch,now=()=>new Date()}){
  ...
  if(typeof d.timezone!=='string'||!d.timezone.trim()||typeof d.utc_offset_seconds!=='number'||!Number.isFinite(d.utc_offset_seconds)||Math.abs(d.utc_offset_seconds)>50400)throw new Error('Open-Meteo operations forecast UTC offset is invalid.');
  const localNow=new Date(Math.ceil((now().getTime()+d.utc_offset_seconds*1000)/3600000)*3600000).toISOString().slice(0,13)+':00';
