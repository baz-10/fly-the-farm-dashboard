...
function assertOperationsForecast(snapshot,retrievedAt){
  ...
  const daily=snapshot.daily||{},dailyTimes=daily.time,dailyFields={weather_code:[0,99],temperature_2m_min:[-100,100],temperature_2m_max:[-100,100],precipitation_probability_max:[0,100],precipitation_sum:[0,10000],precipitation_hours:[0,24],wind_speed_10m_max:[0,500],wind_gusts_10m_max:[0,500],wind_direction_10m_dominant:[0,360]};
  if(!Array.isArray(dailyTimes)||dailyTimes.length<14||!Object.entries(dailyFields).every(([key,range])=>Array.isArray(daily[key])&&daily[key].length===dailyTimes.length&&daily[key].every(value=>Number.isFinite(value)&&value>=range[0]&&value<=range[1])))throw new Error('Open-Meteo operations forecast must return fourteen aligned finite daily values.');
  previous='';for(const date of dailyTimes){if(typeof date!=='string'||!/^\d{4}-\d{2}-\d{2}$/.test(date)||date<=previous)throw new Error('Open-Meteo operations forecast must return fourteen aligned finite daily values.');previous=date;}
...
function buildRainWindows(hours,nextHour){const windows=[];for(const hour of hours.filter(item=>item.rainProbability>=50&&item.rainAmountMm>0)){const previous=windows[windows.length-1];if(previous&&previous.end===hour.time){previous.end=nextHour(hour.time);previous.peakProbability=Math.max(previous.peakProbability,hour.rainProbability);previous.expectedAmountMm=Math.round((previous.expectedAmountMm+hour.rainAmountMm)*10)/10;}else windows.push({certainty:'LIKELY',start:hour.time,end:nextHour(hour.time),peakProbability:hour.rainProbability,expectedAmountMm:Math.round(hour.rainAmountMm*10)/10});}return windows;}
async function fetchOpenMeteoOperationsForecast({latitude,longitude,fetchImpl=global.fetch,now=()=>new Date()}){
  ...
  const query=new URLSearchParams({latitude:String(latitude),longitude:String(longitude),timezone:'auto',forecast_days:'14',current:'temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m,wind_gusts_10m,wind_direction_10m,precipitation,rain,is_day',hourly:'temperature_2m,relative_humidity_2m,precipitation_probability,precipitation,cloud_cover,is_day,wind_speed_10m,wind_gusts_10m,wind_direction_10m',daily:'weather_code,temperature_2m_min,temperature_2m_max,precipitation_probability_max,precipitation_sum,precipitation_hours,wind_speed_10m_max,wind_gusts_10m_max,wind_direction_10m_dominant'});
  ...
  const compass=deg=>['N','NE','E','SE','S','SW','W','NW'][Math.round(Number(deg||0)/45)%8],condition=code=>code===0?'Clear':code<=2?'Partly cloudy':code===3?'Overcast':code<=48?'Fog':code<=57?'Drizzle':code<=67?'Rain':code<=77?'Snow':code<=82?'Showers':code<=86?'Snow showers':code>=95?'Thunderstorms':'Mixed conditions',nextHour=value=>new Date(new Date(`${value}:00Z`).getTime()+3600000).toISOString().slice(0,16),allHourly=(d.hourly?.time||[]).map((time,i)=>{const deltaT=calculateDeltaT(d.hourly.temperature_2m[i],d.hourly.relative_humidity_2m[i]),trend=i?d.hourly.temperature_2m[i]-d.hourly.temperature_2m[i-1]:0,isDay=d.hourly.is_day?.[i],inversionPotential=assessInversionPotential({isDay:isDay===0?false:isDay===1?true:undefined,windSpeedKmh:d.hourly.wind_speed_10m[i],cloudCoverPercent:d.hourly.cloud_cover?.[i],humidityPercent:d.hourly.relative_humidity_2m[i],temperatureTrendC:trend});return{time,temperatureC:d.hourly.temperature_2m[i],humidityPercent:d.hourly.relative_humidity_2m[i],windSpeedKmh:d.hourly.wind_speed_10m[i],windGustKmh:d.hourly.wind_gusts_10m[i],windDirection:compass(d.hourly.wind_direction_10m[i]),isDay:isDay===1,rainProbability:d.hourly.precipitation_probability[i],rainAmountMm:d.hourly.precipitation[i],deltaTC:deltaT,inversionPotential,sprayCondition:assessSprayCondition({deltaT,windSpeedKmh:d.hourly.wind_speed_10m[i],windGustKmh:d.hourly.wind_gusts_10m[i],rainProbability:d.hourly.precipitation_probability[i]})};}),matchedIndex=allHourly.findIndex(item=>item.time>=localNow),startIndex=matchedIndex<0?allHourly.length:matchedIndex,hourly=allHourly.slice(startIndex,startIndex+25),currentDelta=calculateDeltaT(d.current.temperature_2m,d.current.relative_humidity_2m),daily=(d.daily?.time||[]).map((date,i)=>{const dayHours=allHourly.filter(item=>item.time.startsWith(date)),rainWindows=buildRainWindows(dayHours,nextHour),futureDayHours=dayHours.filter(item=>item.isDay&&(date!==localNow.slice(0,10)||item.time>=localNow));return{date,conditionCode:d.daily.weather_code?.[i],condition:condition(d.daily.weather_code?.[i]),minTemperatureC:d.daily.temperature_2m_min[i],maxTemperatureC:d.daily.temperature_2m_max[i],rainProbability:d.daily.precipitation_probability_max[i],rainAmountMm:d.daily.precipitation_sum[i],rainDurationHours:d.daily.precipitation_hours?.[i]??0,windSpeedKmh:d.daily.wind_speed_10m_max[i],windGustKmh:d.daily.wind_gusts_10m_max[i],windDirection:compass(d.daily.wind_direction_10m_dominant?.[i]),confidence:i<7?'STANDARD':'EXTENDED',rainWindows,rainWindow:rainWindows[0]||null,bestSprayWindow:findBestSprayWindow(futureDayHours)};}),currentInversion=hourly[0]?.inversionPotential;
