...
class OperationalRepository {
  ...
  async readReportOutput(context,artefactId){const rows=await supabaseRequest(`rest/v1/report_output_files?${tenantFilter(context)}&artefact_id=eq.${encodeURIComponent(artefactId)}&select=internal_file_id,file_version,original_filename,content_type,byte_size,sha256_checksum,storage_bucket,provider_key&limit=1`,{publicMessage:'Report output could not be loaded.'});return rows?.[0]||null;}
