...
class OperationalRepository {
  ...
  mapMissionCloseoutResult(result){if(result?.conflict)return{conflict:true,currentVersion:result.current_version};for(const key of['not_found','location_forbidden','not_authorised','evidence_incomplete','aircraft_days_incomplete','flight_lines_required','personnel_required','attribution_invalid','forbidden'])if(result?.[key])return{[key.replace(/_([a-z])/g,(_,c)=>c.toUpperCase())]:true};return{record:result?.record||result?.records||result};}
