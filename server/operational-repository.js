...
class OperationalRepository {
  ...
  async stageCustomerAcceptanceFile({organisationId,missionId,token,kind,fileName,contentType,bytes}){const tokenHash=crypto.createHash('sha256').update(token).digest('hex'),internalFileId=crypto.randomUUID(),checksum=crypto.createHash('sha256').update(bytes).digest('hex'),bucket='customer-acceptance-evidence',safeName=fileName.replace(/[^A-Za-z0-9._-]/g,'_'),providerKey=`${organisationId}/${missionId}/${internalFileId}/${safeName}`;await supabaseRequest(`storage/v1/object/${bucket}/${providerKey}`,{method:'POST',body:bytes,headers:{'Content-Type':contentType,'x-upsert':'false'},publicMessage:'Signature could not be stored.'});try{const rows=await supabaseRequest('rest/v1/customer_acceptance_files',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({organisation_id:organisationId,mission_id:missionId,internal_file_id:internalFileId,kind,original_filename:fileName,content_type:contentType,byte_size:bytes.length,sha256_checksum:checksum,storage_provider:'supabase',storage_bucket:bucket,provider_key:providerKey,provenance:{channel:'SECURE_LINK',capturedAt:new Date().toISOString()},upload_token_hash:tokenHash}),publicMessage:'Signature evidence could not be retained.'});return{record:Array.isArray(rows)?rows[0]:rows};}catch(error){await supabaseRequest(`storage/v1/object/${bucket}/${providerKey}`,{method:'DELETE',publicMessage:'Signature cleanup failed.'}).catch(()=>{});throw error;}}
