...
test('groups frozen Properties and Fields and compares planned to actual chemicals', () => {
  ...
  expect(model.operatingDays[0].weather).toMatchObject({ sourceType: 'OPEN_METEO', coverageStatus: 'ACTUAL_INTERVAL', gaps: [{ reason: 'Provider gap' }] });
...
test.each([
  ['null final completion timestamp',draft=>{draft.finalCompletion.completedAt=null;}],
  ['calendar-invalid final completion timestamp',draft=>{draft.finalCompletion.completedAt='2026-02-30T10:00:00Z';}],
  ['null governance decision timestamp',draft=>{draft.reportEvidence.governance.effectiveApproval.decidedAt=null;draft.reportEvidence.governance.decisionHistory[0].decidedAt=null;}],
  ['missing required weather coordinate',draft=>{delete draft.dailyEvidence.days[0].weatherReport.latitude;}],
  ['null weather observation timestamp',draft=>{draft.dailyEvidence.days[0].weatherReport.hourly_observations=[{observedAt:null,temperatureC:24.375,relativeHumidity:83.25,dewPointC:21.125,windSpeedKmh:13.9,windDirectionDegrees:182.75,precipitationMm:0.05}];}],
])('fails closed on required canonical evidence: %s',(_label,mutate)=>{const draft=clone(document);mutate(draft);expect(buildMissionSummaryViewModel({...input,frozenReportDocument:transportFor(draft)})).toMatchObject({evidenceStatus:'MISSING_FROZEN_EVIDENCE',operatingDays:[]});});
