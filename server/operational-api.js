...
function createReportsHandler(dependencies={}){const repository=dependencies.repository||new OperationalRepository(),getContext=dependencies.resolveContext||resolveRequestContext;return async function(req,res){res.setHeader('Cache-Control','no-store');res.setHeader('Content-Type','application/json; charset=utf-8');try{const context=await getContext(req,res),missionId=assertUuid(req.query?.missionId,'missionId'),mission=await repository.get('missions',context,missionId);if(!mission||!hasAssignedLocationReadAccess('missions',context,mission))throw apiError(404,'NOT_FOUND','Mission not found.');const action=req.query?.action||'history';if(req.method==='GET'){assertPermission(context,'reports','read');const reportType=String(req.query?.reportType||'');if(!['MISSION_PACK','MISSION_RECORD'].includes(reportType))throw apiError(400,'VALIDATION_ERROR','reportType is invalid.');return res.status(200).json({data:await repository.readReportArtefacts(context,missionId,reportType)});}if(req.method!=='POST')throw apiError(405,'METHOD_NOT_ALLOWED','Method not allowed.');assertSameOrigin(req);assertLocationAccess(context,mission.operating_location_id,'Mission');if(action!=='request')throw apiError(400,'UNSUPPORTED_ACTION','Unsupported Reports action.');assertPermission(context,'reports','generate');const body=parseBody(req),reportType=String(body.reportType||'');if(!['MISSION_PACK','MISSION_RECORD'].includes(reportType))throw apiError(400,'VALIDATION_ERROR','reportType is invalid.');assertPermission(context,reportType==='MISSION_PACK'?'mission.pack':'mission.record','generate');if(typeof body.idempotencyKey!=='string'||body.idempotencyKey.length<4||body.idempotencyKey.length>128)throw apiError(400,'VALIDATION_ERROR','idempotencyKey is required.');const result=await repository.requestReportArtefact(context,missionId,reportType,body.idempotencyKey);if(result.completionRequired)throw apiError(409,'COMPLETION_REQUIRED','Complete the Mission before generating a Mission Record.');if(result.notFound)throw apiError(404,'NOT_FOUND','Report evidence was not found.');if(result.locationForbidden)throw apiError(403,'LOCATION_FORBIDDEN','Mission location is not assigned.');return res.status(201).json({data:result});}catch(error){const{status,response}=errorEnvelope(error);return res.status(status).json(response);}};}
