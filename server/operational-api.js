...
function createCustomerAcceptanceHandler(dependencies={}){const repository=dependencies.repository||new OperationalRepository(),getContext=dependencies.resolveContext||resolveRequestContext;return async function(req,res){res.setHeader('Cache-Control','no-store');res.setHeader('Content-Type','application/json; charset=utf-8');try{const context=await getContext(req,res),missionId=assertUuid(req.query?.missionId,'missionId'),mission=await repository.get('missions',context,missionId);if(!mission||!hasAssignedLocationReadAccess('missions',context,mission))throw apiError(404,'NOT_FOUND','Mission not found.');if(req.method==='GET'){assertPermission(context,'mission.customer_acceptance','read');return res.status(200).json({data:await repository.readCustomerAcceptance(context,missionId)});}if(req.method!=='POST')throw apiError(405,'METHOD_NOT_ALLOWED','Method not allowed.');assertSameOrigin(req);assertLocationAccess(context,mission.operating_location_id,'Mission');const action=req.query?.action,body=parseBody(req,MAX_IMPORT_BODY_BYTES);let result;if(action==='record'){assertPermission(context,'mission.customer_acceptance','record');for(const field of['stateCode','methodCode','customerContactName','acknowledgedAt'])if(typeof body[field]!=='string'||!body[field].trim())throw apiError(400,'VALIDATION_ERROR',`${field} is required.`);result=await repository.createCustomerAcceptance(context,missionId,body);}else if(action==='link-issue'){assertPermission(context,'mission.customer_acceptance','link.issue');if(typeof body.contactName!=='string'||!body.contactName.trim())throw apiError(400,'VALIDATION_ERROR','contactName is required.');result=await repository.issueCustomerAcceptanceLink(context,missionId,body);}else if(action==='link-revoke'){assertPermission(context,'mission.customer_acceptance','link.revoke');result=await repository.revokeCustomerAcceptanceLink(context,missionId,body);}else throw apiError(400,'UNSUPPORTED_ACTION','Unsupported Customer Acceptance action.');if(result?.completion_required)throw apiError(409,'COMPLETION_REQUIRED','Complete the Mission before recording customer acceptance.');if(result?.validation_error)throw apiError(400,'VALIDATION_ERROR','Customer Acceptance evidence is incomplete.');if(result?.conflict)throw apiError(409,'VERSION_CONFLICT','The secure link changed.');return res.status(201).json({data:{...(result.record||result),...(result.token?{token:result.token}:{})}});}catch(error){const{status,response}=errorEnvelope(error);return res.status(status).json(response);}};}

function createCustomerAcceptancePublicHandler(dependencies={}){const repository=dependencies.repository||new OperationalRepository();return async function(req,res){res.setHeader('Cache-Control','no-store');res.setHeader('Content-Type','application/json; charset=utf-8');try{const token=typeof req.query?.token==='string'?req.query.token:'';if(token.length<6||token.length>256)throw apiError(404,'LINK_UNAVAILABLE','Customer acceptance link is unavailable.');const fingerprint=crypto.createHash('sha256').update(`${req.headers?.['x-forwarded-for']||''}|${req.headers?.['user-agent']||''}`).digest('hex');if(req.method==='GET'){const result=await repository.resolveCustomerAcceptanceLink(token,fingerprint);if(result?.invalid||result?.revoked||result?.expired||result?.consumed)throw apiError(410,'LINK_UNAVAILABLE','This customer acceptance link is no longer available.');if(result?.rate_limited)throw apiError(429,'RATE_LIMITED','Too many link requests.');const{organisationName,missionReference,completedAt,customerName,intendedContactName,expiresAt,states}=result;return res.status(200).json({data:{organisationName,missionReference,completedAt,customerName,intendedContactName,expiresAt,states}});}if(req.method!=='POST')throw apiError(405,'METHOD_NOT_ALLOWED','Method not allowed.');assertSameOrigin(req);const action=req.query?.action,body=parseBody(req,MAX_IMPORT_BODY_BYTES);let result;if(action==='signature'){const match=typeof body.dataUrl==='string'&&body.dataUrl.match(/^data:(image\/(?:png|jpeg|webp));base64,([A-Za-z0-9+/=]+)$/);if(!match)throw apiError(400,'VALIDATION_ERROR','A PNG, JPEG or WebP signature is required.');const bytes=Buffer.from(match[2],'base64');if(!bytes.length||bytes.length>3145728)throw apiError(400,'VALIDATION_ERROR','Signature size is invalid.');const link=await repository.resolveCustomerAcceptanceLink(token,fingerprint);if(!link?.linkId)throw apiError(410,'LINK_UNAVAILABLE','This customer acceptance link is no longer available.');result=await repository.stageCustomerAcceptanceFile({organisationId:link.organisationId,missionId:link.missionId,token,kind:'SIGNATURE',fileName:String(body.fileName||'signature.png'),contentType:match[1],bytes});}else if(action==='submit'){for(const field of['stateCode','customerContactName','consentDeclaration','signatureFileId'])if(typeof body[field]!=='string'||!body[field].trim())throw apiError(400,'VALIDATION_ERROR',`${field} is required.`);if(body.consent!==true)throw apiError(400,'VALIDATION_ERROR','Explicit consent is required.');result=await repository.submitCustomerAcceptanceLink(token,fingerprint,body);}else throw apiError(400,'UNSUPPORTED_ACTION','Unsupported customer link action.');if(result?.unavailable)throw apiError(410,'LINK_UNAVAILABLE','This customer acceptance link is no longer available.');if(result?.validation_error)throw apiError(400,'VALIDATION_ERROR','Customer acknowledgement is incomplete.');return res.status(201).json({data:result.record||result});}catch(error){const{status,response}=errorEnvelope(error);return res.status(status).json(response);}};}

module.exports = { createChemicalReviewsHandler, createCustomerAcceptanceHandler, createCustomerAcceptancePublicHandler, createFieldBoundaryVersionHandler, createMissionAuthorisationHandler, createMissionOperationalCloseoutHandler, createMissionOutcomesHandler, createMissionChemicalsHandler, createMissionJsaHandler, createMissionMapHandler, createMissionPersonnelHandler, createMissionWeatherHandler, createOperationalHandler, createPersonnelHandler, createPersonnelIdentityHandler, createSessionHandler, errorEnvelope, mapBoundaryRecord, mapDatabaseRecord, mapMissionMapSourceFileRecord, mapPersonnelRecord };
