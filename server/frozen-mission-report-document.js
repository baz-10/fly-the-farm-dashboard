...
function bounded(value,depth=0,counter={nodes:0}){if(depth>12||++counter.nodes>50000)return false;if(value===null||typeof value==='boolean')return true;if(typeof value==='number')return Number.isFinite(value);if(typeof value==='string')return value.length<=10000&&!/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(value);if(Array.isArray(value))return value.length<=10000&&value.every(item=>bounded(item,depth+1,counter));if(!value||typeof value!=='object'||Object.keys(value).length>100)return false;return Object.entries(value).every(([key,item])=>key.length<=100&&bounded(item,depth+1,counter));}
...
function numeric(value,min,max){return value===null||(typeof value==='number'&&Number.isFinite(value)&&value>=min&&value<=max);}
function coordinate(value,min,max){if(typeof value==='string'&&!/^-?(?:0|[1-9]\d{0,2})\.\d{1,6}$/.test(value))return false;const parsed=typeof value==='string'?Number(value):value;return numeric(parsed,min,max);}
function weatherObservation(item){const keys=['observedAt','temperatureC','relativeHumidity','dewPointC','windSpeedKmh','windDirectionDegrees','precipitationMm'];return item&&typeof item==='object'&&!Array.isArray(item)&&Object.keys(item).length===7&&keys.every(key=>Object.hasOwn(item,key))&&timestamp(item.observedAt)&&numeric(item.temperatureC,-100,100)&&numeric(item.relativeHumidity,0,100)&&numeric(item.dewPointC,-150,100)&&numeric(item.windSpeedKmh,0,500)&&numeric(item.windDirectionDegrees,0,359.999999)&&numeric(item.precipitationMm,0,10000)&&keys.slice(1).some(key=>item[key]!==null);}
function weatherGap(item){return item&&typeof item==='object'&&!Array.isArray(item)&&Object.keys(item).length===2&&timestamp(item.observedAt)&&string(item.reason,1000)&&item.reason===item.reason.trim();}
function decode(transport){
  ...
  for(const day of daily.days){
    ...
    const weather=day.weatherReport;if(weather.operating_day_id!==day.id||weather.mission_id!==final.missionId||weather.mission_pack_revision_id!==day.packageRevisionId||!string(weather.source_type)||!string(weather.coverage_status)||!Array.isArray(weather.hourly_observations)||!Array.isArray(weather.coverage_gaps)||weather.hourly_observations.length>48||weather.coverage_gaps.length>48||weather.hourly_observations.some(item=>!weatherObservation(item))||weather.coverage_gaps.some(item=>!weatherGap(item))||(weather.latitude!==undefined&&!coordinate(weather.latitude,-90,90))||(weather.longitude!==undefined&&!coordinate(weather.longitude,-180,180))||(['interval_start_at','interval_end_at','provider_retrieved_at'].some(key=>weather[key]!==undefined&&!timestamp(weather[key]))))return invalid();
