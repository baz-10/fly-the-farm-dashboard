...
class SupportRepository {
  ...
  async resolveSession(sessionId,platformUserId){
    ...
    const rows=await supabaseRequest(`rest/v1/support_sessions?id=eq.${encodeURIComponent(sessionId)}&platform_user_id=eq.${encodeURIComponent(platformUserId)}&select=${encodeURIComponent(select)}&limit=1`,{publicMessage:'Delegated Support Session could not be loaded.'});const row=rows?.[0];if(!row)return null;const approvals=row.support_requests?.support_approval_events||[],approval=approvals.filter(x=>x.decision==='APPROVE').sort((a,b)=>String(b.approval_timestamp).localeCompare(String(a.approval_timestamp)))[0];return{id:row.id,organisationId:row.organisation_id,organisationName:row.organisations?.name||'Supported organisation',platformUserId:row.platform_user_id,accessMode:row.access_mode,scopeType:row.scope_type,missionId:row.mission_id,jobId:row.job_id,moduleCode:row.module_code,reason:row.reason,state:row.state,startedAt:row.started_at,expiresAt:row.expires_at,approvedByInternalUserId:approval?.approved_by_internal_user_id||null};
  }
