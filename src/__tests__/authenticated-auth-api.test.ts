...
describe('Supabase authentication API', () => {
  ...
  test.each([
    ['unapproved applicant', 'new@example.com'],
    ['allow-listed owner', 'ben@flythefarm.com.au'],
  ])('denies direct public registration from an %s without creating an organisation', async (_description, email) => {
    global.fetch = jest.fn() as any;
    const res = createResponse();

    await authHandler({
      method: 'POST', headers: { host: 'localhost:3001' },
      body: { action: 'register', email, name: 'New Operator', password: 'password', role: 'contractor' },
    }, res);

    expect(res.statusCode).toBe(403);
    expect(res.body.error).toBe('Self-service account registration is not available. Apply for access.');
    expect(global.fetch).not.toHaveBeenCalled();
  });
