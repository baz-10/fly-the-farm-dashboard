...
test('captures direct, inherited, default and effective ftf_store privilege provenance read-only', () => {
  ...
  expect(sql).toMatch(/if\s+v_default_maintain\s+then[\s\S]*governed default MAINTAIN present/i);
  expect(sql).not.toMatch(/v_direct_maintain\s+is\s+distinct\s+from\s+v_default_maintain/i);
