...
export function createPersonnelApi(fetcher:Fetcher=fetch){return{
 ...
 createCasaCredential:async(id:string,input:Record<string,unknown>&{file:File})=>{const{file,...values}=input;return request(fetcher,'/api/v1/personnel-casa-credentials?action=create',{method:'POST',body:JSON.stringify({...values,personnelId:id,file:{fileName:file.name,contentType:file.type,sizeBytes:file.size,dataUrl:await asDataUrl(file)}})});},
 verifyCasaCredential:(credentialId:string,expectedVersion:number,decision:string,notes:string)=>request(fetcher,'/api/v1/personnel-casa-credentials?action=verify',{method:'POST',body:JSON.stringify({credentialId,expectedVersion,decision,notes})}),
