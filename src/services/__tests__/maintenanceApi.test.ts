...
describe('maintenanceApi',()=>{beforeEach(()=>{global.fetch=jest.fn();});
  ...
  test('normalizes every Fleet row and preserves server counts and filters',async()=>{
    (fetch as jest.Mock).mockResolvedValue(response(200,{data:{
      ...
      rows:[{registryId:'33333333-3333-4333-8333-333333333333',source:'fleet-asset',sourceRecordId:'fleet-1',identity:'FTF-11',operatingLocationId:'44444444-4444-4444-8444-444444444444',highestState:'DUE_SOON',requirementCount:1,attachedAssetCount:0,stateCounts:{CURRENT:0,DUE_SOON:1,DUE:0,OVERDUE:0,INSUFFICIENT_DATA:0},dueState:dueResult()}],
    }}));
    const summary=await maintenanceApi.readFleetDueSummary('2026-08-21T01:30:00.000Z',{baseId:'44444444-4444-4444-8444-444444444444',assetType:'fleet-asset',state:'DUE_SOON'});
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining('action=fleet-due-summary&asOf=2026-08-21T01%3A30%3A00.000Z&baseId=44444444-4444-4444-8444-444444444444&assetType=fleet-asset&state=DUE_SOON'),expect.objectContaining({method:'GET'}));
    expect(summary.rows[0].dueState.assetId).toBe(summary.rows[0].registryId);
  });

  test('fails the whole Fleet response if one nested projection is unsafe',async()=>{
    (fetch as jest.Mock).mockResolvedValue(response(200,{data:{asOf:'2026-08-21T01:30:00.000Z',filters:{baseId:null,assetType:null,state:null},counts:{CURRENT:1,DUE_SOON:0,DUE:0,OVERDUE:0,INSUFFICIENT_DATA:0},rows:[{registryId:'33333333-3333-4333-8333-333333333333',source:'fleet-asset',sourceRecordId:'fleet-1',identity:'FTF-11',operatingLocationId:'44444444-4444-4444-8444-444444444444',highestState:'CURRENT',requirementCount:1,attachedAssetCount:0,stateCounts:{CURRENT:1,DUE_SOON:0,DUE:0,OVERDUE:0,INSUFFICIENT_DATA:0},dueState:dueResult({serviceability:'unserviceable'})}]}}));
    await expect(maintenanceApi.readFleetDueSummary('2026-08-21T01:30:00.000Z')).rejects.toMatchObject({code:'MALFORMED_RESPONSE'});
  });

  test('fails the whole Fleet response when compact metadata contradicts its authoritative projection',async()=>{
    (fetch as jest.Mock).mockResolvedValue(response(200,{data:{asOf:'2026-08-21T01:30:00.000Z',filters:{baseId:null,assetType:null,state:null},counts:{CURRENT:1,DUE_SOON:0,DUE:0,OVERDUE:0,INSUFFICIENT_DATA:0},rows:[{registryId:'33333333-3333-4333-8333-333333333333',source:'fleet-asset',sourceRecordId:'fleet-1',identity:'FTF-11',operatingLocationId:'44444444-4444-4444-8444-444444444444',highestState:'CURRENT',requirementCount:99,attachedAssetCount:0,stateCounts:{CURRENT:99,DUE_SOON:0,DUE:0,OVERDUE:0,INSUFFICIENT_DATA:0},dueState:dueResult()}]}}));
    await expect(maintenanceApi.readFleetDueSummary('2026-08-21T01:30:00.000Z')).rejects.toMatchObject({code:'MALFORMED_RESPONSE'});
  });
