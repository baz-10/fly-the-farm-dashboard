...
test('strictly decodes bounded revision history and sends exact Slice 5 commands',async()=>{const history={schemaVersion:'FINANCIAL_ACTUAL_REVISION_HISTORY_V1',financialActualId:ID,reference:'FA-000001',archivedAt:null,currentFinalRevisionId:ID,activeDraftRevisionId:null,rows:[{id:ID,revisionNumber:1,status:'FINAL',rowVersion:3,predecessorRevisionId:null,correctionReason:null,formulaVersion:'FINANCIAL_ACTUAL_V1',finalisedAt:'2026-08-20T00:00:00Z',finalisedByInternalUserId:ID,current:true,activeDraft:false}],nextBeforeRevisionNumber:null};const correction={schemaVersion:'FINANCIAL_ACTUAL_CORRECTION_V1',record:{id:ID,reference:'FA-000001',rowVersion:4,currentFinalRevisionId:ID,activeDraftRevisionId:ID},revision:{id:ID,revisionNumber:2,status:'DRAFT',rowVersion:1,predecessorRevisionId:ID,correctionReason:'Correct product cost.'}},archive={schemaVersion:'FINANCIAL_ACTUAL_ARCHIVE_V1',record:{id:ID,reference:'FA-000001',rowVersion:5,archivedAt:'2026-08-22T00:00:00Z',currentFinalRevisionId:ID}};const fetcher=jest.fn().mockImplementationOnce(()=>ok(history)).mockImplementationOnce(()=>ok(correction)).mockImplementationOnce(()=>ok(archive));const api=createFinancialActualsApi(fetcher as any);await expect(api.revisionHistory({actualId:ID,pageSize:25})).resolves.toEqual(history);await expect(api.createCorrection({actualId:ID,expectedAggregateVersion:3,expectedFinalRevisionId:ID,expectedFinalRevisionVersion:3,correctionReason:'Correct product cost.'})).resolves.toEqual(correction);await expect(api.archive({actualId:ID,expectedAggregateVersion:4,archiveReason:'No longer operational'})).resolves.toEqual(archive);expect(JSON.parse(fetcher.mock.calls[1][1].body).correctionReason).toBe('Correct product cost.');});

test('fails the whole revision history for extra authority or contradictory current state',async()=>{const malformed={schemaVersion:'FINANCIAL_ACTUAL_REVISION_HISTORY_V1',financialActualId:ID,reference:'FA-000001',archivedAt:null,currentFinalRevisionId:ID,activeDraftRevisionId:null,rows:[{id:ID,revisionNumber:1,status:'FINAL',rowVersion:3,predecessorRevisionId:null,correctionReason:null,formulaVersion:'FINANCIAL_ACTUAL_V1',finalisedAt:'2026-08-20T00:00:00Z',finalisedByInternalUserId:ID,current:false,activeDraft:false,secret:'leak'}],nextBeforeRevisionNumber:null};await expect(createFinancialActualsApi(jest.fn().mockResolvedValue(await ok(malformed)) as any).revisionHistory({actualId:ID})).rejects.toThrow('Financial Actual data could not be validated.');});

test('fails whole for extra fields in frozen MANUAL revenue provenance',async()=>{const revision:any={id:ID,revisionNumber:1,status:'FINAL',rowVersion:3,predecessorRevisionId:null,correctionReason:null,currencyCode:'AUD',formulaVersion:'FINANCIAL_ACTUAL_V1',startDate:'2026-08-20',endDate:'2026-08-20',input:{formulaVersion:'FINANCIAL_ACTUAL_V1',currencyCode:'AUD',revenue:{mode:'MANUAL',manualRevenue:'3.0000',provenance:{fieldPath:'revenue/manualRevenue',provenanceClass:'MANUAL_FINANCIAL_INPUT',effectiveValue:'3.0000',unitCode:'AUD',secret:'must fail'}},workEntries:[],costLines:[],workEntryEvidence:[],costLineEvidence:[]},provenance:{rows:[]},calculation:{formulaVersion:'FINANCIAL_ACTUAL_V1',currencyCode:'AUD',operationalDays:0,totalHours:'0.0000',revenue:'3.0000',totalCost:'0.0000',grossProfit:'3.0000',grossMarginPercentage:'100.0000',effectiveHourlyRevenue:null,categoryTotals:{LABOUR:'0.0000',PRODUCT:'0.0000',TRAVEL:'0.0000',AIRCRAFT_EQUIPMENT:'0.0000',OTHER:'0.0000'},lineAmounts:{}},sourceManifest:{schemaVersion:'FINANCIAL_ACTUAL_SOURCE_MANIFEST_V1',financialActualId:ID,revisionId:ID,formulaVersion:'FINANCIAL_ACTUAL_V1',revenueMode:'MANUAL',workEntryIds:[],costLineIds:[],provenanceIds:[]},inputDigest:'a'.repeat(64),finalisedAt:'2026-08-20T00:00:00Z',finalisedByInternalUserId:ID};const body={schemaVersion:'FINANCIAL_ACTUAL_HISTORICAL_REVISION_V1',financialActualId:ID,reference:'FA-000001',archivedAt:null,current:true,revision};await expect(createFinancialActualsApi(jest.fn().mockResolvedValue(await ok(body)) as any).historicalRevision({actualId:ID,revisionId:ID})).rejects.toThrow('Financial Actual data could not be validated.');});

test('downloads only a bounded authoritative PDF with a safe filename',async()=>{const bytes=new Blob(['%PDF exact'],{type:'application/pdf'}),response={ok:true,status:200,headers:new Headers({'content-type':'application/pdf','content-length':String(bytes.size),'content-disposition':'attachment; filename="FA-000001-revision-1.pdf"'}),blob:async()=>bytes}as Response,fetcher=jest.fn().mockResolvedValue(response),api=createFinancialActualsApi(fetcher as any);await expect(api.exportFinal({actualId:ID,revisionId:ID})).resolves.toMatchObject({filename:'FA-000001-revision-1.pdf'});expect(fetcher).toHaveBeenCalledWith('/api/v1/financial-actuals?action=export',expect.objectContaining({method:'POST',credentials:'same-origin',body:JSON.stringify({actualId:ID,revisionId:ID})}))});

test.each([['text/html','attachment; filename="FA-000001-revision-1.pdf"'],['application/pdf','attachment; filename="../secret.pdf"']])('rejects malformed export response %s',async(type,disposition)=>{const bytes=new Blob(['not authoritative'],{type}),response={ok:true,status:200,headers:new Headers({'content-type':type,'content-length':String(bytes.size),'content-disposition':disposition}),blob:async()=>bytes}as Response;await expect(createFinancialActualsApi(jest.fn().mockResolvedValue(response)as any).exportFinal({actualId:ID,revisionId:ID})).rejects.toMatchObject({code:'FINANCIAL_ACTUAL_EXPORT_UNAVAILABLE'})});
