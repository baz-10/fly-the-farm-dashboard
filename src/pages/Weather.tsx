...
export default function Weather() {
  ...
  return <Box sx={{ maxWidth: 1500, mx: 'auto', p: { xs: 2, md: 3.5 } }}>
    ...
    {weather.forecast && current && <Stack spacing={2}>
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}><Typography fontWeight={900} mb={1.5}>Hourly forecast</Typography><Box sx={{ overflowX: 'auto' }}><Stack direction="row" spacing={1}>{weather.forecast.hourly.map((hour) => <Paper key={hour.time} variant="outlined" sx={{ p: 1.25, minWidth: 130 }}><Typography fontWeight={800}>{format(new Date(hour.time), 'ha')}</Typography><Typography>{hour.tempC}°C</Typography><Typography variant="caption">ΔT {hour.deltaT} · {hour.windSpeedKmh} km/h</Typography></Paper>)}</Stack></Box></Paper>
