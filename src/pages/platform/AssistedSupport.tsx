...
export default function AssistedSupport(){const[items,setItems]=React.useState<any[]>([]);const[error,setError]=React.useState('');
 ...
 const start=async(id:string)=>{try{const session=await startSupportSession(id);const target=session.scope_type==='MISSION'&&session.mission_id?`/missions/${session.mission_id}`:'/jobs';window.location.assign(target);}catch(e){setError(e instanceof Error?e.message:'Session could not start.');}};
