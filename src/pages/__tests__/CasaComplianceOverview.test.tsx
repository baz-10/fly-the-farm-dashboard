...
test('uses operational status instead of unexplained scoring fractions',async()=>{
 render(<CasaComplianceOverview/>);
 await screen.findByText('92%');
 expect(screen.queryByText('0 / 20')).not.toBeInTheDocument();
 expect(screen.queryByText('0 / 15')).not.toBeInTheDocument();
 expect(screen.getByText('Your ReOC certificate evidence is missing.')).toBeInTheDocument();
});

test('shows an upcoming dated obligation as due soon rather than missing',async()=>{
 render(<CasaComplianceOverview/>);
 await screen.findByText('92%');
 const upcoming=screen.getByRole('heading',{name:'Next 90 days'}).closest('.MuiCard-root');
 expect(upcoming).not.toBeNull();
 expect(within(upcoming as HTMLElement).getByText('Due soon')).toBeInTheDocument();
 expect(within(upcoming as HTMLElement).getByText(/15 days remaining/)).toBeInTheDocument();
 expect(within(upcoming as HTMLElement).queryByText('Missing')).not.toBeInTheDocument();
});
