...
function InvitationEvidence({ invitation, onRevoke }: { invitation: CommercialInvitationEvidence; onRevoke: () => void }) {
  ...
  return <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, p: 2 }}>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" gap={1}>
      ...
      {(effectiveStatus === 'SENT' || effectiveStatus === 'PENDING') && <Button size="small" color="error" variant="outlined" onClick={onRevoke}>Revoke invitation</Button>}
    </Stack>
    <Grid container spacing={2} sx={{ mt: 0.5 }}>
      <Grid size={{ xs: 12, sm: 4 }}><EvidenceLabel label="Sent">{formatTime(invitation.sentAt || invitation.createdAt)}</EvidenceLabel></Grid>
...
export default function CommercialOnboardingReview() {
  ...
  const [invitationPath, setInvitationPath] = React.useState('');

  const load = React.useCallback(async () => {
    try { setApplications(await listCommercialApplications()); setError(''); }
  ...
  const confirm = async () => {
    ...
    try {
      if (confirmation.kind === 'approve' || confirmation.kind === 'decline') {
        ...
        const result = await issueCommercialInvitation({ applicationId: confirmation.application.id, expectedVersion: confirmation.application.rowVersion, notes: notes.trim(), resend: confirmation.kind === 'resend' });
  ...
  const dialogAction = confirmation?.kind === 'approve' ? 'Confirm approval'
    ...
    : confirmation?.kind === 'revoke' ? 'Confirm revocation' : 'Confirm and create invitation';
  ...
  return <Stack spacing={2.5} sx={{ mt: 4 }}>
    ...
    {invitationPath && <Alert severity="success">
      <Typography fontWeight={750}>One-time acceptance path created</Typography>
      <TextField fullWidth size="small" value={invitationPath} slotProps={{ input: { readOnly: true } }} sx={{ mt: 1, bgcolor: 'background.paper' }} />
      <Typography variant="caption">Copy this path now. The raw token is not stored and cannot be shown again.</Typography>
    </Alert>}
          ...
          <CardContent sx={{ p: { xs: 2.5, md: 3 } }}>
            <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" gap={2}>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ sm: 'center' }}>
                {application.status === 'SUBMITTED' && <Button variant="contained" disabled={working} onClick={() => void beginReview(application)}>Start review</Button>}
                ...
                {application.status === 'APPROVED' && !activeInvitation && <Button variant="contained" startIcon={<ForwardToInboxRoundedIcon />} onClick={() => open({ kind: mayResend ? 'resend' : 'issue', application })}>{mayResend ? 'Send another invitation' : 'Send invitation'}</Button>}
              </Stack>
            </Stack>
            ...
            <Grid container spacing={2.5}>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                ...
                {application.base ? <Stack spacing={1.3}>
                  ...
                  <EvidenceLabel label="Provenance">{formatStatus(application.base.addressSource)} · {application.base.timezone}</EvidenceLabel>
            ...
            {application.invitations.length > 0 && <Stack spacing={1.5} sx={{ mt: 2.5 }}>
              ...
              {application.invitations.map((invitation) => <InvitationEvidence key={invitation.id} invitation={invitation} onRevoke={() => open({ kind: 'revoke', application, invitation })} />)}
            </Stack>}
          </CardContent>
        </Card>;
      })}
      ...
      <DialogContent>
        <DialogContentText sx={{ mb: 2 }}>
          {confirmation?.kind === 'approve' ? 'Approval records an authoritative decision. It does not create or send an invitation.'
            ...
            : 'This creates a one-time acceptance path from the approved snapshot. Copy it before closing the success message.'}
        </DialogContentText>
