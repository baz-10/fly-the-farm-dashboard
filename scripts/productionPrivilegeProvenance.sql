...
begin
  ...
  if v_acl is distinct from array['postgres=arwdDxtm/postgres','service_role=arwdm/postgres']::text[] then
  ...
  select count(*) into v_default_maintain
  from pg_catalog.pg_default_acl def
  cross join lateral aclexplode(def.defaclacl) acl
  where acl.grantee=(select oid from pg_catalog.pg_roles where rolname='service_role')
    and acl.privilege_type='MAINTAIN';
  if v_default_maintain<>0 then
    raise exception 'PRODUCTION_PRIVILEGE_PROVENANCE: default MAINTAIN authority present';
  end if;
  ...
  if not has_table_privilege('service_role','public.ftf_store','select,insert,update,delete')
    or not has_table_privilege('service_role','public.ftf_store','maintain')
...
with recursive membership_chain as (
  ...
  select role_name,privilege,
    has_table_privilege(role_name,'public.ftf_store',privilege) as allowed
...
select jsonb_build_object(
  ...
  'defaultTablePrivileges',coalesce((select jsonb_agg(to_jsonb(defaults) order by owner,schema,object_type,privilege_type) from defaults),'[]'::jsonb),
