...
Every assessed item resolves to exactly one repository-controlled state, factor and reason using this precedence: Operationally blocking `0.00`; Expired or overdue `0.00`; Missing `0.00`; Under review `0.50`; Due within 30 days `0.60`; Due within 90 days `0.80`; Current `1.00`; Superseded or Not applicable, excluded. State bands are mutually exclusive, so an item due within 30 days is never also counted within 90 days. A category earns its weight multiplied by the arithmetic mean of its applicable item factors. The returned category result includes weight, earned points, assessed/current/due-soon/missing/expired/blocking counts and authorised source references. Overall percentage is earned points divided by applicable weight; a category with no applicable items is excluded only when the underlying requirement is genuinely not applicable. A missing required baseline creates a Missing item with factor zero.
...
Every returned critical blocker includes its rule code and rule version, human-readable reason, source entity type and ID, source row/version identifier where available, organisation and operating-location scope, affected operation or compliance area, authorised application route where available and evaluation timestamp. The UI exposes this provenance directly; an unexplained critical state is invalid.
