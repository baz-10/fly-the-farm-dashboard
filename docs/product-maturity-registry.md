...
App route completeness and guard/maturity composition are discovered from React Router `Route` JSX in the TypeScript AST rather than source-text matching. Reachable route paths must be quoted strings, JSX string expressions, or no-substitution templates, and the resulting path multiset must exactly match `REACHABLE_PRODUCT_ROUTES`; dynamic paths, spread attributes, missing leaf paths, omissions, extras, and duplicate-count differences fail release verification. Public Login, Auth Callback, Forgot Password, and Reset Password routes must retain their exact auth-lifecycle components; public Register and Customer Acceptance routes must retain `ProductRouteSurface`. The platform layout must keep `PlatformProtectedRoute` outside `ProductRouteSurface`, and organisation product leaves must call the approved `productRoute(...)` helper beneath the exact `ProtectedRoute`/provider layout. The helper and canonical `AuthorisedProductRoute` implementation are also audited so the access guard remains outside the maturity surface. Bare destinations, missing boundaries, reversed ordering, equivalent unsupported wrappers, and additional pathless layouts fail closed. Approved pathless layout children may remain direct or wrapped in one or more JSX fragments.

Rendered copy analysis also inspects method receivers. It statically evaluates supported string/array `concat` calls, array `join` calls, and string `replace`/`replaceAll` calls with fully resolved string receivers, searches, and replacements, including supported static call chains and quoted or no-substitution-template element access, without evaluating application code. String transforms model first-match versus all-match behaviour and the `$$`, `$&`, `` $` ``, and `$'` replacement tokens. A regular expression, callback, dynamic operand, or partially resolved receiver is never executed or guessed: a rendered transform with a fully static receiver fails closed when its search or replacement cannot be safely modeled, while existing dynamic-receiver UI remains outside static transform evaluation. Transparent parenthesized and TypeScript assertion/non-null wrappers around property or element-access method references are removed before method classification. Typed static string and string-array values preserve JavaScript coercion: string `concat` converts an array argument to its comma-joined string form, array `concat` flattens string-array arguments by one level, and `join` applies its resolved separator before the result is checked. Dynamic rendered element-access method names, including wrapped references, and unresolved rendered `join` or `concat` calls with static receiver copy fail closed, while non-rendered calls remain outside customer-copy analysis and all existing candidate, depth, node, symbol, and cycle budgets still apply.
