...
The CI verifier validates registry completeness, structured Founder approval, reviewable evidence paths and unique module/workflow keys; checks reachable App routes against the route manifest; and uses the TypeScript AST to reject `Legacy` customer copy across production TSX sources in `src/pages`, `src/components`, and `src/navigation`. JSX text, rendered expressions (including logical/conditional and nested call/object/array forms), imported constant copy, customer-copy properties, visible/accessibility attributes, and customer-message notice/toast/setter calls are checked. Comments, internal identifiers/strings, and structural JSX attributes such as `id`, `className`, and `data-*` are ignored. Server, persistence, migration, and internal engineering sources are outside that customer-UI scan. A verification failure blocks the release workflow until the governance issue is corrected. It does not change application runtime permissions, workflow triggers, environments, secrets, or operational acceptance credentials.
