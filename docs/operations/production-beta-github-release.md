...
The workflow then starts the same checkout with `githubCommitSha=RELEASE_SHA` as Vercel metadata and server-only `SPRAY_COMMAND_RELEASE_SHA=RELEASE_SHA` runtime configuration. The pinned CLI uses `--no-wait`, returns the generated deployment URL immediately after creation and inspects that URL for deployment ID, creation timestamp and state before the explicit `READY` wait. Those facts therefore survive a later build or wait failure. After `READY`, the workflow retrieves the exact deployment through the authenticated Vercel API and fails on missing, invalid or mismatched `meta.githubCommitSha`.

Only after that independent metadata check does the pinned, protection-aware `vercel curl --deployment` command query `/api/v1/deployment` on the newly created deployment URL. That endpoint reads only `SPRAY_COMMAND_RELEASE_SHA` and must return the same SHA. The workflow then inspects `spray-command-production-beta.vercel.app` and requires its resolved deployment ID to equal the newly created deployment ID. Operational acceptance cannot start until both the generated-deployment runtime proof and this canonical-alias join succeed.
...
- Any failure after the migration boundary is a **PARTIAL RELEASE**. The canonical record names whether migration apply, ledger reconciliation, deployment creation, deployment identity capture, `READY` wait, metadata verification, generated-deployment runtime verification or canonical-alias verification failed. Preserve applied migration history and keep the previous application serving where Vercel supports it.
- Missing or mismatched Vercel deployment metadata stops before the runtime identity query; missing or mismatched generated-deployment runtime identity, or a canonical alias that does not resolve to the newly created deployment ID, stops acceptance and fails the release.
