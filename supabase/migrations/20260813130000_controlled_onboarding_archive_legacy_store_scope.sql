...
begin
  if jsonb_typeof(p_evidence)<>'object' then
    raise exception 'COMMERCIAL_ONBOARDING_ACCEPTANCE_EVIDENCE_INVALID: legacyStore projection'
      using errcode='22023';
  end if;
  begin
  ...
  then
    raise exception 'COMMERCIAL_ONBOARDING_ACCEPTANCE_PROVENANCE_MISMATCH: legacyStore projection'
      using errcode='55000';
  end if;
  ...
  if v_count<>1
    or (select count(*) from public.ftf_store where tenant_id=v_organisation_id)<>1
  then
...
revoke all on function public.ftf_project_controlled_onboarding_legacy_store(jsonb)
  from public,anon,authenticated,service_role;
grant execute on function public.ftf_project_controlled_onboarding_legacy_store(jsonb)
  to service_role;
