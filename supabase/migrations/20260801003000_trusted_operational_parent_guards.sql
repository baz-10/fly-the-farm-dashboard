-- Corrective forward migration: relationship parents are validated and locked in
-- the same transaction as the trusted operational write. field_boundary_versions
-- and job_fields are intentionally not exposed as RPC resources in this slice.
...
declare
  ...
  v_table text;
begin
  if p_resource not in ('clients', 'properties', 'fields', 'jobs', 'missions') or p_operation not in ('create', 'update', 'archive') then raise exception 'unsupported operational resource write'; end if;
  if not exists (select 1 from public.internal_users iu join public.memberships m on m.organisation_id = iu.organisation_id and m.internal_user_id = iu.id where iu.organisation_id = p_organisation_id and iu.id = p_actor_internal_user_id and iu.is_active and iu.archived_at is null and m.is_active and m.archived_at is null) then raise exception 'active organisation actor required' using errcode = '42501'; end if;
  v_table := p_resource;

  if p_operation <> 'create' then
    if p_entity_id is null or p_expected_version is null or p_expected_version < 1 then raise exception 'entity id and expected version are required'; end if;
    execute format('select row_version, archived_at from public.%I where organisation_id = $1 and id = $2 for update', v_table) into v_current_version, v_archived_at using p_organisation_id, p_entity_id;
    ...
    if p_operation = 'archive' and (
      (p_resource = 'clients' and (exists (select 1 from public.properties where organisation_id = p_organisation_id and client_id = p_entity_id and archived_at is null) or exists (select 1 from public.jobs where organisation_id = p_organisation_id and client_id = p_entity_id and archived_at is null))) or
      (p_resource = 'properties' and (exists (select 1 from public.fields where organisation_id = p_organisation_id and property_id = p_entity_id and archived_at is null) or exists (select 1 from public.field_boundary_versions where organisation_id = p_organisation_id and property_id = p_entity_id and archived_at is null) or exists (select 1 from public.jobs where organisation_id = p_organisation_id and property_id = p_entity_id and archived_at is null))) or
      (p_resource = 'fields' and exists (select 1 from public.job_fields where organisation_id = p_organisation_id and field_id = p_entity_id and archived_at is null)) or
      (p_resource = 'jobs' and (exists (select 1 from public.job_fields where organisation_id = p_organisation_id and job_id = p_entity_id and archived_at is null) or exists (select 1 from public.missions where organisation_id = p_organisation_id and job_id = p_entity_id and archived_at is null))) or
      (p_resource = 'missions' and exists (select 1 from public.mission_versions where organisation_id = p_organisation_id and mission_id = p_entity_id and archived_at is null))
    ) then return jsonb_build_object('archive_conflict', true); end if;
  end if;

  -- Parent guards run for every create and update, including updates that do
  -- not alter a relationship. FOR UPDATE makes a concurrent archive wait.
  if p_operation <> 'archive' and p_resource = 'properties' then
    perform 1 from public.clients where organisation_id = p_organisation_id and id = (p_data->>'client_id')::uuid and archived_at is null for update;
    if not found then return jsonb_build_object('relationship_conflict', true); end if;
  elsif p_operation <> 'archive' and p_resource = 'fields' then
    perform 1 from public.properties where organisation_id = p_organisation_id and id = (p_data->>'property_id')::uuid and archived_at is null for update;
    if not found then return jsonb_build_object('relationship_conflict', true); end if;
    if nullif(p_data->>'field_boundary_version_id', '') is not null then
      perform 1 from public.field_boundary_versions where organisation_id = p_organisation_id and id = (p_data->>'field_boundary_version_id')::uuid and property_id = (p_data->>'property_id')::uuid and archived_at is null for update;
      if not found then return jsonb_build_object('relationship_conflict', true); end if;
    end if;
  elsif p_operation <> 'archive' and p_resource = 'jobs' then
    perform 1 from public.clients where organisation_id = p_organisation_id and id = (p_data->>'client_id')::uuid and archived_at is null for update;
    if not found then return jsonb_build_object('relationship_conflict', true); end if;
    perform 1 from public.properties where organisation_id = p_organisation_id and id = (p_data->>'property_id')::uuid and client_id = (p_data->>'client_id')::uuid and archived_at is null for update;
    if not found then return jsonb_build_object('relationship_conflict', true); end if;
  elsif p_operation <> 'archive' and p_resource = 'missions' then
    perform 1 from public.jobs where organisation_id = p_organisation_id and id = (p_data->>'job_id')::uuid and archived_at is null for update;
    if not found then return jsonb_build_object('relationship_conflict', true); end if;
    perform 1 from public.operating_locations where organisation_id = p_organisation_id and id = (p_data->>'operating_location_id')::uuid and archived_at is null for update;
    if not found then return jsonb_build_object('relationship_conflict', true); end if;
  end if;

  if p_operation = 'archive' then
    execute format('update public.%I set archived_at = now(), archived_by_internal_user_id = $1 where organisation_id = $2 and id = $3 and row_version = $4 and archived_at is null returning to_jsonb(%I)', v_table, v_table) into v_record using p_actor_internal_user_id, p_organisation_id, p_entity_id, p_expected_version;
