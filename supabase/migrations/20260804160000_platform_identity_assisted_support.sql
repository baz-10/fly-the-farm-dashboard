...
begin
 ...
 insert into public.audit_events(organisation_id,event_type,entity_type,entity_id,event_payload)values(v_session.organisation_id,'support.session_started','support_session',v_session.id,jsonb_build_object('platformUserId',p_platform_user_id,'expiresAt',v_session.expires_at));
 ...
 return jsonb_build_object('session_id',v_session.id,'state',v_session.state,'started_at',v_session.started_at,'expires_at',v_session.expires_at,'access_mode',v_session.access_mode,'scope_type',v_session.scope_type);
