...
begin
  ...
  v_manifest_digest:=encode(sha256(convert_to(new.daily_evidence_manifest::text,'UTF8')),'hex');
  ...
  new.report_document_digest:=encode(sha256(convert_to(v_text,'UTF8')),'hex');
  return new;
end $$;
